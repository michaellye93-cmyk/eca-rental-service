...
const zeroTotals = (): FinanceTotals => ({ cash: 0, revenue: 0, service_claim: 0, commission: 0, recurring: 0, workshop: 0, insurance: 0, direct_costs: 0, contribution: 0, margin: null });
...
export function calculateFinance(input: FinanceInput) {
  ...
  if (![1,2].includes(calculationVersion)) throw new Error('Unsupported Finance calculation version; preserve the original version when reading closed months.');
  ...
  for (const cost of input.recurring_costs) if (monthOverlaps(cost, month)) contributionFor(cost.plate_key).recurring += cost.monthly_amount;
  ...
  let corporateOpex = 0;
  for (const expense of input.expenses.filter((entry) => monthKey(entry.finance_month) === month)) {
  ...
  for (const vehicle of contributions.values()) {
    if (vehicle.business_unit !== 'UNMATCHED' && vehicle.revenue > 0 && !input.recurring_costs.some((cost) => normalizePlate(cost.plate_key) === vehicle.plate_key && monthOverlaps(cost, month))) issues.push({ code: 'MISSING_COST_MASTER', severity: 'warning', detail: 'Vehicle has revenue but no applicable recurring cost master', plate_key: vehicle.plate_key });
  ...
  return { vehicles: [...contributions.values()], businesses, totals, corporate_opex: cents(corporateOpex), management_profit: cents(totals.contribution - corporateOpex), issues };
}
