...
- Reproduced the exact `Enter valid coverage dates` error for VPE6727 using an XLSX formula with no cached value before changing production code.
...
- Read the user-supplied `ECA_Management_PnL_Raw_Data_Template.xlsx`: VPE6727 parses as OWNER_PAID, premium 0, null start/end/payment dates and no issues. All 12 owner-paid RM0 rows have zero coverage-date errors.
