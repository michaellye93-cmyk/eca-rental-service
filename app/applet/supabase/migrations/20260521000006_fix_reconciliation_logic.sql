...
BEGIN
    ...
    LOOP
        ...
        -- 1. STRONGEST: Match by exact car plate in reference or sender_name
        SELECT id, car_plate
        INTO matched_driver
        FROM drivers
        WHERE 
        ...
        -- 3. MEDIUM: Match an ALREADY EXISTING payment by amount & date (with name fallback)
        IF matched_driver IS NULL THEN
        ...
        -- 4. WEAK: Check for matching Rental rate as absolute fallback if no text matches?
        IF matched_driver IS NULL THEN
            SELECT id, car_plate
            INTO matched_driver
            FROM drivers
