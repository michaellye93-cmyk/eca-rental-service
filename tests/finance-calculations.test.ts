...
const input = (overrides: Partial<FinanceInput> = {}): FinanceInput => ({
  month, vehicles: [{ plate_key: 'VGA4412', display_plate: 'VGA 4412', business_unit: 'E-HAILING', ownership_type: 'Owned', status: 'Active' }],
...
test('normalizes plate keys by uppercasing and removing whitespace', () => {
  assert.equal(normalizePlate(' vga  4412 '), 'VGA4412');
...
test('owner-paid insurance has no ECA cost and an ECA-paid renewal uses its own premium and coverage',()=>{
 const owner={plate_key:'VGA4412',premium:1200,payment_date:null,coverage_start:'2026-01-01',coverage_end:'2026-12-31',responsibility:'OWNER_PAID' as const};
...
test('flags changed bank matches and preserves unknown recurring classifications for review',()=>{
 const report=calculateFinance(input({recurring_costs:[{plate_key:'VGA4412',start_month:'2026-08-01',end_month:null,cost_type:'Owner Payout / Loan - To Classify',monthly_amount:100,payee:null,notes:null}],bank_rows:[{import_id:'bank1',source_row:1,transaction_date:'2026-08-01',description:'Loan',reference:null,debit:100,credit:0,decision:'MATCHED',payment_source:null,category:null,plate_key:'VGA4412',matched_kind:'recurring_cost',matched_id:'c1',review_note:'',match_valid:false}]}));
...
test('calculates e-hailing revenue as cash plus claim while treating the claim as maintenance', () => {
  const report = calculateFinance(input({ ehailing: [{ source_payment_id: 'p1', driver_id: 'd1', driver_name_snapshot: null,
    car_plate_snapshot: 'VGA 4412', plate_key: 'vga 4412', payment_date: '2026-08-03', cash_amount: 150, service_claim: 300,
...
test('applies inclusive recurring month boundaries and leaves corporate opex out of vehicles', () => {
  const report = calculateFinance(input({ recurring_costs: [{ plate_key: 'VGA4412', start_month: '2026-08', end_month: '2026-08',
...
test('allocates a full calendar-year premium evenly and balances to the premium', () => {
  const insurance = { plate_key: 'VGA4412', premium: 1440, payment_date: null, coverage_start: '2026-01-01', coverage_end: '2026-12-31' };
...
test('accepts canonical database month dates while the UI supplies a year-month', () => {
  const policy = { plate_key: 'VGA4412', premium: 1440, payment_date: null, coverage_start: '2026-01-01', coverage_end: '2026-12-31' };
  const report = calculateFinance(input({ month: { ...month, finance_month: '2026-08-01' }, recurring_costs: [{ plate_key: 'VGA4412', start_month: '2026-08-01', end_month: '2026-08-01', cost_type: 'Zero-cost contract', monthly_amount: 0, payee: null, notes: null }], insurance: [policy] }));
...
test('reports snapshot controls and editable historical attribution even on first ingestion', () => {
  const report = calculateFinance(input({ month: { ...month, source_count: 2, total_cash: 99, total_claim: 2 }, smart_import: null, ehailing: [{ source_payment_id: 'p1', driver_id: 'd1', driver_name_snapshot: '', car_plate_snapshot: 'VGA 4412', plate_key: 'VGA4412', payment_date: '2026-08-01', cash_amount: 100, service_claim: 2, gross_rental_revenue: 102, payment_method: null, refreshed_at: '', finance_month: '2026-08-01', attribution_changed: false }] }));
...
test('uses coverage fractions for partial months and keeps the premium balanced', () => {
  const insurance = { plate_key: 'VGA4412', premium: 310, payment_date: null, coverage_start: '2026-01-16', coverage_end: '2026-02-14' };
