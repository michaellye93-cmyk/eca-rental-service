...
test('section parser preserves source audit, accepts normal cents, and flags invalid values without dropping rows', async () => {
 ...
 assert.equal(preview.rows.length, 2); assert.equal(preview.rows[0].amount, 19.99); assert.deepEqual(preview.rows[0].source_row, 2); assert.equal(preview.issues.some(x => x.code === 'INVALID_SECTION_ROW'), true);
});
