...
test('complete ledger ingestion, close freezing, stale revision denial and explicit reopen',async()=>{
 const db=await setup();try{
  await db.exec(`insert into public.drivers values('10000000-0000-4000-8000-000000000001','Test Driver','VGA 4412');
  ...
  await db.query(`select public.finance_save_record('vehicle',$1::jsonb)`,[JSON.stringify({plate_key:'VGA4412',display_plate:'VGA 4412',business_unit:'E-HAILING',ownership_type:'Owned',status:'Active'})]);
  await db.query(`select public.finance_save_record('recurring_cost',$1::jsonb)`,[JSON.stringify({plate_key:'VGA4412',start_month:month,end_month:null,cost_type:'Owner Payout',monthly_amount:100,payee:null,notes:null})]);
  ...
  await asUser(db);
  await db.query(`select public.finance_save_record('vehicle',$1::jsonb)`,[JSON.stringify({plate_key:'VGA4412',display_plate:'V G A 4412',business_unit:'SMART DRIVE',ownership_type:'Owned',status:'Active'})]);
