...
const vehicles: FinanceVehicle[] = [{ plate_key: 'VGA4412', display_plate: 'VGA 4412', business_unit: 'DAILY RENTAL', ownership_type: 'Owned', status: 'Active' }];
...
test('reads explicit sheets and strips customer identity fields from generic inspection', async () => {
  const buffer = await workbook([{ name: 'Sales Report', headers: ['Customer Name', 'Car Plate', 'Revenue'], rows: [['Private Person', 'VGA 4412', 100]] }]);
  const result = await readFinanceWorkbook(buffer);
  assert.deepEqual(result.sheets[0].headers, ['Car Plate', 'Revenue']);
  assert.deepEqual(result.sheets[0].rows[0], { source_row: 2, sheet_name: 'Sales Report', values: { 'Car Plate': 'VGA 4412', Revenue: 100 } });
});

test('previews Smart Drive report with finance fields, vehicle matching, and a confirmed report month', async () => {
  const buffer = await workbook([{ name: 'Sales Report', headers: ['Car Plate', 'Start Date', 'End Date', 'Revenue', 'Commission Paid', 'Transaction Status/Date'], rows: [['VGA 4412', '30/08/2026', '31/08/2026', 100, 30, '2026-08-31']] }]);
  ...
  assert.deepEqual(preview.rows[0], { source_row: 2, sheet_name: 'Sales Report', reference: null, plate_key: 'VGA4412', display_plate: 'VGA 4412', pickup_date: '2026-08-30', return_date: '2026-08-31', gross_revenue: 100, commission: 30, status: '2026-08-31', payment_status: null });
});
...
test('recalculates Smart Drive row and distinct vehicle matches on every parse of the same workbook', async () => {
  const buffer = await workbook([{name:'Sales',headers:['Car Plate','Start Date','End Date','Revenue','Commission Paid','Status'],rows:['vga 4412','VGA4412','new 123','NEW123',''].map(plate=>[plate,'2026-08-01','2026-08-02',100,10,'Completed'])}]);
...
test('rejects missing headers, status, invalid dates and inconsistent money without dropping rows', async () => {
  const buffer = await workbook([{ name: 'Sales Report', headers: ['Car Plate', 'Start Date', 'End Date', 'Revenue', 'Commission Paid', 'Transaction Status/Date'], rows: [['VGA 4412', '31/02/2026', '01/02/2026', -1, 2, '']] }]);
...
test('keeps source audit fields and canonical database months in bootstrap and workshop previews', async () => {
  const buffer = await workbook([
    { name: 'Vehicle Master', headers: ['Car Plate', 'Business Unit', 'Ownership Type', 'Status'], rows: [['VGA 4412', 'E-HAILING', 'Owned', 'Active']] },
    { name: 'Vehicle Monthly Costs', headers: ['Car Plate', 'Start Month', 'End Month', 'Cost Type', 'Monthly Amount', 'Payee', 'Notes'], rows: [['VGA 4412', '2026-08', '', 'Loan', 0, 'Lender Name', '']] },
    { name: 'Vehicle Insurance', headers: ['Car Plate', 'Premium', 'Payment Date', 'Coverage Start', 'Coverage End'], rows: [['VGA 4412', 1440, '2026-01-01', '2026-01-01', '2026-12-31']] },
  ]);
  ...
  const workshop = await previewWorkshop(await workbook([{ name: 'Workshop', headers: ['Billing Date', 'Car Plate', 'Supplier Name', 'Amount'], rows: [['2026-08-15', 'VGA 4412', 'Vendor', 20]] }]), 'workshop.xlsx', '2026-08', vehicles);
...
test('keeps every row in a report larger than the usual API page size and flags duplicate references', async () => {
  const rows = Array.from({ length: 1001 }, (_, index) => [`B${index === 1000 ? 0 : index}`, 'VGA 4412', '2026-08-01', '2026-08-02', 10, 3]);
...
test('imports bootstrap master, recurring cost, and insurance sheets', async () => {
  const buffer = await workbook([
    { name: 'Vehicle Master', headers: ['Car Plate', 'Business Unit', 'Ownership Type', 'Status'], rows: [['VGA 4412', 'E-HAILING', 'Owned', 'Active']] },
    { name: 'Monthly Costs', headers: ['Car Plate', 'Start Month', 'End Month', 'Cost Type', 'Monthly Amount', 'Payee', 'Notes'], rows: [['VGA 4412', '2026-08', '', 'Loan', 100, 'Bank', '']] },
    { name: 'Insurance', headers: ['Car Plate', 'Premium', 'Payment Date', 'Coverage Start', 'Coverage End', 'RESPONSIBILITY'], rows: [['VGA 4412', 1440, '2026-01-01', '2026-01-01', '2026-12-31', 'ECA PAID']] },
  ]);
...
test('keeps unclassified recurring source labels and warns without moving their start month', async () => {
  const buffer = await workbook([
    { name: 'Vehicle Master', headers: ['Car Plate', 'Business Unit', 'Ownership Type', 'Status'], rows: [['VGA 4412', 'E-HAILING', 'Owned', 'Active']] },
    { name: 'Vehicle Monthly Costs', headers: ['Car Plate', 'Start Month', 'Cost Type', 'Monthly Amount'], rows: [['VGA 4412', '2026-09-01', 'Owner Payout / Loan - To Classify', 100]] },
...
test('parses workshop rows without customer fields and requires finance month, amount, and plate', async () => {
  const buffer = await workbook([{ name: 'Workshop', headers: ['Billing Date', 'Car Plate', 'Supplier', 'Amount', 'Reference', 'Description'], rows: [['15/08/2026', 'VGA 4412', 'Workshop A', 80, 'W1', 'Oil']] }]);
